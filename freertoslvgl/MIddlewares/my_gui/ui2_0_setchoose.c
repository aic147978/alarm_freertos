...
static lv_obj_t* make_btn(const char *txt)
{
 ...
 //   lv_obj_remove_style_all(b);
    ...
    lv_obj_set_style_bg_color(b, lv_color_hex(COL_PR),     LV_PART_MAIN | LV_STATE_PRESSED);
    lv_obj_set_style_bg_color(b, lv_color_hex(COL_PR),     LV_PART_MAIN | LV_STATE_CHECKED);
    lv_obj_set_style_bg_color(b, lv_color_hex(0x7A8CCF),   LV_PART_MAIN | LV_STATE_DISABLED); /* 可选：禁用色 */
    ...
    lv_obj_set_style_text_color(lab, lv_color_hex(COL_TXT), 0); /* 文字颜色放在 label 上 */
    lv_obj_center(lab);
    return b;
}
