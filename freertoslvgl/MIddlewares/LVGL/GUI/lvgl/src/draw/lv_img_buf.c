...
{
    const uint8_t * src_uint8_t = (const uint8_t *)dsc->cfg.src;
    ...
    if(dsc->tmp.native_color) {
        if(dsc->tmp.has_alpha == 0) {
            ...
            lv_memcpy_small(&dsc->res.color, &src_uint8_t[pxi], px_size);
        }
        else {
            px_size = LV_IMG_PX_SIZE_ALPHA_BYTE;
            pxi     = dsc->cfg.src_w * ys_int * px_size + xs_int * px_size;
            lv_memcpy_small(&dsc->res.color, &src_uint8_t[pxi], px_size - 1);
            dsc->res.opa = src_uint8_t[pxi + px_size - 1];
        }
    }
    else {
        pxi = 0; /*unused*/
...
{
    const uint8_t * src_uint8_t = dsc->cfg.src;
    ...
    if(dsc->tmp.native_color) {
        lv_memcpy_small(&c01, &src_uint8_t[dsc->tmp.pxi + dsc->tmp.px_size * xn], sizeof(lv_color_t));
        lv_memcpy_small(&c10, &src_uint8_t[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn], sizeof(lv_color_t));
        lv_memcpy_small(&c11, &src_uint8_t[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn + dsc->tmp.px_size * xn],
                        sizeof(lv_color_t));
        if(dsc->tmp.has_alpha) {
            a10 = src_uint8_t[dsc->tmp.pxi + dsc->tmp.px_size * xn + dsc->tmp.px_size - 1];
            a01 = src_uint8_t[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn + dsc->tmp.px_size - 1];
            a11 = src_uint8_t[dsc->tmp.pxi + dsc->cfg.src_w * dsc->tmp.px_size * yn + dsc->tmp.px_size * xn + dsc->tmp.px_size - 1];
        }
    }
    else {
