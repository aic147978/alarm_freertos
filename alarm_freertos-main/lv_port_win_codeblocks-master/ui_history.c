...
#define RIGHT_PANEL_W       80    /* 右侧按钮区域宽度 */
#define MARGIN_LR           20
#define MARGIN_TB           20
...
static void fill_feed_page(void)
{
    ...
    lv_table_set_col_width(s_table_feed, 0, 120);
    lv_table_set_col_width(s_table_feed, 1, 140);
    lv_table_set_col_width(s_table_feed, 2, 180);
    lv_table_set_col_width(s_table_feed, 3, 180);
...
static void adjust_row_height(lv_obj_t *table){
    if(!table) return;

    /* 表格可见的总行数：表头1 + 数据行 ROWS_PER_PAGE */
    const uint16_t rows_total = ROWS_PER_PAGE + 1;

    const lv_font_t *font = lv_obj_get_style_text_font(table, LV_PART_ITEMS);
    int fh = font ? font->line_height : 16;   // 兜底 16

    lv_coord_t H = lv_obj_get_height(table);

    /* 预估每行的目标高度（整除即可，最后几像素由 LVGL 自己处理） */
    int target_row_h = (H > 0) ? (H / rows_total) : (fh + 2);

    /* 需要的 padding = (目标行高 - 字体行高) / 2，至少为 0 */
    int pad = target_row_h - fh;
    if(pad < 0) pad = 0;
    pad /= 2;

    /* 对所有单元格（含表头）统一设置上下 padding */
    lv_obj_set_style_pad_top(table,    pad, LV_PART_ITEMS);
    lv_obj_set_style_pad_bottom(table, pad, LV_PART_ITEMS);

    /* 如果你希望表头更窄/更高，可另外给表头设置 TYPE 后再单独 style，这里先统一 */
}
...
void history_ui_refresh(void)
{
    ...
    adjust_row_height(s_table_alarm);
    adjust_row_height(s_table_feed);
...
static void create_tables(lv_obj_t *parent)
{
    ...
    no_scroll(s_table_alarm);
no_scroll(s_table_feed);

 /*       lv_obj_clear_flag(s_table_alarm, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_clear_flag(s_table_feed,  LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_clear_flag(s_table_alarm, LV_OBJ_FLAG_SCROLL_ELASTIC | LV_OBJ_FLAG_SCROLL_MOMENTUM);
    lv_obj_clear_flag(s_table_feed,  LV_OBJ_FLAG_SCROLL_ELASTIC | LV_OBJ_FLAG_SCROLL_MOMENTUM);
    lv_obj_set_scroll_dir(s_table_alarm, LV_DIR_NONE);
    lv_obj_set_scroll_dir(s_table_feed,  LV_DIR_NONE);
    lv_obj_set_scrollbar_mode(s_table_alarm, LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_scrollbar_mode(s_table_feed,  LV_SCROLLBAR_MODE_OFF);*/
    /* 初始只显示报警表 */
    lv_obj_add_flag(s_table_feed, LV_OBJ_FLAG_HIDDEN);
...
void history_ui_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(s_root, 760, 360);
    lv_obj_align(s_root, LV_ALIGN_TOP_LEFT, 0, 0);
...
{
    /* center_container 建议大小 800x400（你之前的布局） */
    history_ui_create(center_container);

    /* 可选：先塞一些示例数据 */
    history_ui_add_alarm(1, "2025-09-19 14:00", "Overload");
    history_ui_add_alarm(2, "2025-09-19 14:10", "Door Open");
        history_ui_add_alarm(3, "2025-09-19 14:00", "Overload");
    history_ui_add_alarm(4, "2025-09-19 14:10", "Door Open");
        history_ui_add_alarm(5, "2025-09-19 14:00", "Overload");
    history_ui_add_alarm(6, "2025-09-19 14:10", "Door Open");
        history_ui_add_alarm(7, "2025-09-19 14:00", "Overload");
    history_ui_add_alarm(8, "2025-09-19 14:10", "Door Open");
    history_ui_add_feed (1, "2025-09-19 14:05", 120, 880);
    history_ui_add_feed (2, "2025-09-19 14:15", 150, 730);

    /* 跳到最新页（默认就是最新页，可不调） */
    history_ui_goto_last_page();
}
