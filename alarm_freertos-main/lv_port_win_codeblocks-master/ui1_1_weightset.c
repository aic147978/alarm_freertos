#include "ui1_1_weightset.h"
#include "keyboard.h"   /* 需要你的数字键盘模块：keyboard_show_with_cb(...) */
#include <stdio.h>
#include <string.h>

/* ================== 位置与尺寸（像素，含中文注释） ================== */
/* —— 标题行纵向位置 —— */
static lv_coord_t TITLE_Y = 10;  /* 标题行整体的 Y（越小越靠上；影响“标题↔第一行”的距离） */

/* —— 左侧五行的上下基线 ——
 * 第 v 行(0..4) 的基线 Y = L_ROWS_TOP_Y + v*(L_ROWS_BOTTOM_Y - L_ROWS_TOP_Y)/4 */
static lv_coord_t L_ROWS_TOP_Y    = 64;   /* 左：第一行基线 Y */
static lv_coord_t L_ROWS_BOTTOM_Y = 350;  /* 左：第五行基线 Y */

/* —— 右侧五行的上下基线 —— */
static lv_coord_t R_ROWS_TOP_Y    = 64;   /* 右：第一行基线 Y */
static lv_coord_t R_ROWS_BOTTOM_Y = 350;  /* 右：第五行基线 Y */

/* —— 左侧三列 X（序号 / set(kg) / time(s)） —— */
static lv_coord_t L_IDX_X  =  16;  /* 左：序号“1.”的 X */
static lv_coord_t L_SET_X  =  60;  /* 左：重量 set 按钮的 X */
static lv_coord_t L_TIME_X = 226;  /* 左：时间 time 按钮的 X */

/* —— 右侧三列 X（序号 / set(kg) / time(s)） —— */
static lv_coord_t R_IDX_X  = 360;  /* 右：序号“1.”的 X */
...
static lv_coord_t CELL_H = 46;     /* 按钮高度 */

/* —— 右侧翻页按钮（中心点绝对定位） —— */
static int PAGER_UP_CX = 748;
static int PAGER_UP_CY = 180;
static int PAGER_DN_CX = 748;
...
static lv_obj_t *s_title_l = NULL, *s_title_r = NULL;  /* 左右标题 */
static lv_obj_t *s_btn_up = NULL, *s_btn_dn = NULL;    /* 翻页 */
static lv_obj_t *s_btn_switch = NULL;                  /* 右下角切换 */

static uint16_t s_rows_total = 7;
static uint16_t s_page = 0;

/* —— 数据（destroy 不清空；再次 create 会保留） —— */
static float s_wL[MAX_ROWS] = {0};  /* 左重量 kg */
static float s_tL[MAX_ROWS] = {0};  /* 左时间 s  */
static float s_wR[MAX_ROWS] = {0};  /* 右重量 kg */
static float s_tR[MAX_ROWS] = {0};  /* 右时间 s  */

static ui11_weightset_toggle_cb_t s_toggle_cb = NULL;
...
/* 格式化：去掉多余 0，例如 1.20kg → 1.2kg，1.00kg → 1kg */
...
static void pager_dn_cb(lv_event_t *e){ LV_UNUSED(e); if(s_page+1<ui11_weightset_get_page_count()){ ++s_page; rebuild_rows(); } }
...
lv_obj_set_style_bg_color(s_root, lv_color_hex(0xF2F6FB), 0); // 你之前的淡蓝背景，可改
...
/* ================== 行数/分页 ================== */
void ui11_weightset_set_row_count(uint16_t rows)
{
    if(rows<1) rows=1; if(rows>MAX_ROWS) rows=MAX_ROWS;
    s_rows_total = rows;
    if(s_page >= ui11_weightset_get_page_count()) s_page = ui11_weightset_get_page_count()? ui11_weightset_get_page_count()-1 : 0;
    rebuild_rows();
}
uint16_t ui11_weightset_get_row_count(void){ return s_rows_total; }
uint16_t ui11_weightset_get_page(void){ return s_page; }
uint16_t ui11_weightset_get_page_count(void){ return (s_rows_total + PAGE_ROWS - 1)/PAGE_ROWS; }
void ui11_weightset_set_page(uint16_t p){ if(p>=ui11_weightset_get_page_count()) p=ui11_weightset_get_page_count()-1; s_page=p; rebuild_rows(); }
void ui11_weightset_next_page(void){ if(s_page+1<ui11_weightset_get_page_count()){ ++s_page; rebuild_rows(); } }
void ui11_weightset_prev_page(void){ if(s_page>0){ --s_page; rebuild_rows(); } }
...
float ui11_weightset_get_left_weight (uint16_t row){ return pick(s_wL,row); }
float ui11_weightset_get_left_time   (uint16_t row){ return pick(s_tL,row); }
float ui11_weightset_get_right_weight(uint16_t row){ return pick(s_wR,row); }
float ui11_weightset_get_right_time  (uint16_t row){ return pick(s_tR,row); }

void  ui11_weightset_set_left_weight (uint16_t row,float v){ poke(s_wL,row,v); rebuild_rows(); }
void  ui11_weightset_set_left_time   (uint16_t row,float v){ poke(s_tL,row,v); rebuild_rows(); }
void  ui11_weightset_set_right_weight(uint16_t row,float v){ poke(s_wR,row,v); rebuild_rows(); }
void  ui11_weightset_set_right_time  (uint16_t row,float v){ poke(s_tR,row,v); rebuild_rows(); }
...
/* ================== 其他 ================== */
void ui11_weightset_set_toggle_cb(ui11_weightset_toggle_cb_t cb, void *user_data){ s_toggle_cb = cb; s_toggle_ud = user_data; }
lv_obj_t* ui11_weightset_root(void){ return s_root; }
...
{ PAGER_UP_CX=up_cx; PAGER_UP_CY=up_cy; PAGER_DN_CX=dn_cx; PAGER_DN_CY=dn_cy; PAGER_D=diameter;
  if(s_btn_up) lv_obj_set_pos(s_btn_up, PAGER_UP_CX - PAGER_D/2, PAGER_UP_CY - PAGER_D/2);
  if(s_btn_dn) lv_obj_set_pos(s_btn_dn, PAGER_DN_CX - PAGER_D/2, PAGER_DN_CY - PAGER_D/2);
  if(s_btn_up) lv_obj_set_size(s_btn_up, PAGER_D, PAGER_D);
  if(s_btn_dn) lv_obj_set_size(s_btn_dn, PAGER_D, PAGER_D);
}
