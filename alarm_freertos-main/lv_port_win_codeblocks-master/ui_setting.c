...
static lv_obj_t *s_manual_container = NULL;  /* manual 页的承载容器 */
static bool      s_manual_created   = false; /* 是否已在该容器里创建过 manual UI */
static bool      s_manual_bound     = false; /* 是否已经把 Start/Stop 输出位绑定给*/
...
static void on_manual_exit(void)
{
    if (s_manual_container) {
        lv_obj_add_flag(s_manual_container, LV_OBJ_FLAG_HIDDEN);  // 隐藏手动界面容器
    }
    settings_ui_show_menu();  // 或者切回你想要的任意子页
}
...
void settings_ui_create(lv_obj_t *parent)
{
    ...
    build_line(s_root);                      /* ← 用真实的 line 子页 */
        ...
        s_manual_container = lv_obj_create(s_root);
    no_scroll(s_manual_container);
    lv_obj_set_size(s_manual_container, LV_PCT(100), LV_PCT(100));
    lv_obj_align(s_manual_container, LV_ALIGN_TOP_LEFT, 0, 0);
    lv_obj_add_flag(s_manual_container, LV_OBJ_FLAG_HIDDEN);
...
void settings_ui_destroy(void)
{
    ui_keypad_close();
        ...
        if(s_manual_created){
        manual_ui_destroy();
        s_manual_created = false;
        s_manual_bound   = false;
    }
       if(s_manual_container){
        lv_obj_del(s_manual_container);
        s_manual_container = NULL;
    }
...
static void show_only(PageId p)
{
    ...
    case PAGE_LINE:     lv_obj_clear_flag(s_line,  LV_OBJ_FLAG_HIDDEN);        lv_obj_move_foreground(s_line);         break;
        case PAGE_MANUAL:
    if(!s_manual_created && s_manual_container){
        manual_ui_create(s_manual_container);
        s_manual_created = true;
    }
        /* 2) 只绑定一次 Start/Stop 输出位地址（★★你必须先定义好那两个变量） */
  //      if(!s_manual_bound){
   //         manual_ui_bind_outputs(&g_manual_start_flag, &g_manual_stop_flag); // ★★ 填你的变量地址
     //       s_manual_bound = true;
      //  }
        /* 3) 每次进入做一次同步（把自动页 set/time 复制到手动副本，不影响自动页） */
    manual_ui_set_exit_cb(on_manual_exit);

    manual_ui_sync_from_auto();  // 你的同步逻辑保留
    lv_obj_clear_flag(s_manual_container, LV_OBJ_FLAG_HIDDEN);
    lv_obj_move_foreground(s_manual_container);
        break;
