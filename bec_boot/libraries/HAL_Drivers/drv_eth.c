...
#ifdef STM32F4
#if LWIP_IGMP
	uint32_t value  = EthHandle.Instance->MACFFR;
	value |= 1UL<<31;
	value |= 1UL<<4;
	value &= ~(1UL<<3);
	EthHandle.Instance->MACFFR = value;
#endif
#endif
...
#if LWIP_PTP
	extern uint32_t ETH_PTPSubSecond2NanoSecond(u32_t SubSecondValue);
	q->time_sec = DmaTxDesc->TimeStampHigh;
	q->time_nsec = ETH_PTPSubSecond2NanoSecond(DmaTxDesc->TimeStampLow);
#endif
...
struct pbuf *rt_stm32_eth_rx(rt_device_t dev)
{
	...
	#if LWIP_PTP
		extern uint32_t ETH_PTPSubSecond2NanoSecond(u32_t SubSecondValue);
		q->time_sec = dmarxdesc->TimeStampHigh;
		q->time_nsec = ETH_PTPSubSecond2NanoSecond(dmarxdesc->TimeStampLow);
	#endif
    }
...
extern void network_link_status_function(int8_t status);
static void phy_linkchange()
{
    ...
    if (phy_speed != phy_speed_new)
    {
        ...
        if (phy_speed & PHY_LINK)
        {
			...
			network_link_status_function(1);
			LOG_I("link up");
        }
        else
        {     
            eth_device_linkchange(&stm32_eth_device.parent, RT_FALSE);
			network_link_status_function(0);
			LOG_I("link down");
        }
    }
}
...
static void phy_monitor_thread_entry(void *parameter)
{
	...
	while(phy_addr == 0xFF){
		for(int i = 0; i <= 0x1F; i ++){ 
			EthHandle.Init.PhyAddress = i;
			if(HAL_ETH_ReadPHYRegister(&EthHandle,PHY_SPECIAL_MODES_REG, &regvalue) != HAL_OK){
...
#else
	if(HAL_OK == HAL_ETH_WritePHYRegister(&EthHandle, PHY_BASIC_CONTROL_REG, PHY_RESET_MASK)){
		HAL_ETH_WritePHYRegister(&EthHandle, PHY_BASIC_CONTROL_REG, PHY_AUTO_NEGOTIATION_MASK);
	}else{
		HAL_ETH_WritePHYRegister(&EthHandle, PHY_BASIC_CONTROL_REG, PHY_RESET_MASK);
		rt_thread_mdelay(2000);
